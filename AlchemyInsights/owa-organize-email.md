---
title: Sähköpostin järjestäminen Outlookin verkkoversio
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.custom: ''
ms.openlocfilehash: 6f776d830fc3e017c8611353f31af8f0164f2f0873d01d0d6bc5bc5f5c92eef5
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: fi-FI
ms.lasthandoff: 08/05/2021
ms.locfileid: "53983090"
---
# <a name="organize-your-email"></a>Sähköpostin järjestäminen

In Outlookin verkkoversio, you can use commands like **Sweep** and **Move to to** automatically delete or move messages from a particular sender. Nämä komennot näkyvät viestiluettelon yläpuolella, kun valitset viestin Saapuneet-kansiossa.

- **Puhdista** <img src='data:image/png;base64,' />
 -toiminnolla voit nopeasti poistaa viestejä Saapuneet-kansiosta. Voit poistaa automaattisesti kaikki tämän lähettäjän saapuvat sähköpostit, säilyttää vain uusimmat sähköpostit ja poistaa loput tai yli 10 päivää vanhat sähköpostit.
- **Siirrä kohteeseen** <img src='data:image/png;base64,' /> -toiminnolla voit siirtää tietyn viestin toiseen kansioon.

Lisätietoja on kohdassa [Viestikansioiden käyttäminen Outlookin verkkoversio.](https://support.office.com/article/ae0f10d6-54e7-4f29-acd3-78cdc3fdcb9f)