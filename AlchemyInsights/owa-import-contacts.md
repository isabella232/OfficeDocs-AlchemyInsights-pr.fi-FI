---
title: '8000019: yhteys tietojen lisääminen tai tuominen'
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "8000019"
- "2002"
ms.assetid: ''
ms.openlocfilehash: 7eb53224bb3370fc434e520d69910d0c9c23f501
ms.sourcegitcommit: c6692ce0fa1358ec3529e59ca0ecdfdea4cdc759
ms.translationtype: MT
ms.contentlocale: fi-FI
ms.lasthandoff: 09/14/2020
ms.locfileid: "47694056"
---
# <a name="import-contacts"></a>Yhteys tietojen tuominen

Ennen kuin tuot yhteys tietoja Outlookin verkko versioon, sinun on vietävä ne toisesta sähköpostipalveluntarjoajastasi. csv-tiedostona. Noudata sitten seuraavia ohjeita.

1. Valitse sivun alareunasta <img src='data:image/png;base64,' />
 **Ihmiset**.
2. Valitse työkalu riviltä **Manage**  >  **tuo yhteys tietojen**hallinta.
3. Valitse **Selaa**, valitse CSV-tiedosto ja valitse sitten **Avaa**.
4. Valitse **tuo**.

Lisä tietoja on artikkelissa [yhteys tietojen tuominen Outlookiin](https://support.office.com/article/bb796340-b58a-46c1-90c7-b549b8f3c5f8#ID0EAACAAA=Outlook_on_the_web).

